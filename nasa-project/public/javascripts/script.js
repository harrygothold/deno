...
function loadLaunches() {
  return fetch("/launches").then((launchesResponse) => launchesResponse.json())
...
function abortLaunch(id) {
  return fetch(`/launches/${id}`, {
    method: "delete",
  }).then(loadLaunches).then(listUpcoming);
}
...
function submitLaunch() {
  ...
  return fetch("/launches", {
    ...
    .then(document.getElementById("launch-success").hidden = false).then(
      loadLaunches,
    );
}
